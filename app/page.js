...
6. Calendario inteligente (costo adicional): relevante con reforma laboral 40hrs — detecta inconsistencias cuando se excede la jornada.
...
async function loadSessions(pin) {
  try {
    const now = Date.now();
    const sessions = [];
    const prefix = STORAGE_PREFIX + pin + ":";
...
async function saveSession(pin, session) {
  try {
    const key = STORAGE_PREFIX + pin + ":" + session.id;
...
function LoginScreen({ onLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "enter") {
      if (!users[cleanPin]) { setError("PIN no encontrado. ¿Es nuevo? Regístrate primero."); setLoading(false); return; }
      const sessions = await loadSessions(cleanPin);
      onLogin({ pin: cleanPin, name: users[cleanPin], sessions });
    } else {
      if (!name.trim()) { setError("Escribe tu nombre."); setLoading(false); return; }
      if (users[cleanPin]) { setError("Ese PIN ya está en uso. Elige otro."); setLoading(false); return; }
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100vh", background: "#f3f4f6", padding: 24 }}>
      <div style={{ background: "white", borderRadius: 20, padding: "40px 36px", width: "100%", maxWidth: 380, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{ background: "#00AEEF", color: "white", border: "none", borderRadius: 10, padding: "12px", fontSize: 14, fontWeight: 700, cursor: "pointer", marginTop: 4, opacity: loading ? 0.7 : 1 }}>
            {loading ? "Verificando..." : mode === "enter" ? "Entrar →" : "Crear cuenta →"}
          </button>
        </div>
      </div>
    </div>
...
function Sidebar({ sessions, currentSessionId, onSelectSession, onNewSession, userName, onLogout }) {
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "4px 8px 16px" }}>
        {sessions.length === 0 ? (
          ...
          sessions.map(s => {
            ...
            return (
              <button key={s.id} onClick={() => onSelectSession(s)} style={{ width: "100%", background: isActive ? "#1f2937" : "transparent", border: isActive ? "1px solid #374151" : "1px solid transparent", borderRadius: 10, padding: "10px 12px", cursor: "pointer", textAlign: "left", marginBottom: 4, transition: "all 0.15s" }}
...
export default function PetraApp() {
  ...
  const handleLogin = ({ pin, name, sessions: loadedSessions }) => {
    setUser({ pin, name });
  ...
  const handleNewSession = () => setCurrentSession(newSession(activeTab));

  const handleSelectSession = (s) => { setCurrentSession(s); setActiveTab(s.tab); };
  ...
  const persistSession = async (updatedSession) => {
    if (!user) return;
    ...
    setSessions(updated);
    await saveSession(user.pin, updatedSession);
  };
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const reply = data.text || "No pude obtener una respuesta. Intenta de nuevo.";
      const finalMessages = [...updatedMessages, { role: "assistant", content: reply }];
      const finalSession = { ...updatedSession, messages: finalMessages };
      setCurrentSession(finalSession);
      await persistSession(finalSession);
    } catch {
      const errorSession = { ...updatedSession, messages: [...updatedMessages, { role: "assistant", content: "Hubo un error de conexión. Por favor intenta de nuevo." }] };
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          {showWelcome ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, textAlign: "center", gap: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 8, width: "100%", maxWidth: 520 }}>
                {suggestions.map((s, i) => (
                  <button key={i} onClick={() => sendMessage(s.text)} style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 12, padding: "14px", cursor: "pointer", textAlign: "left", fontSize: 13, color: "#374151", lineHeight: 1.5, transition: "all 0.15s", display: "flex", gap: 8, alignItems: "flex-start" }}
            ...
            <>
              {messages.map((msg, i) => (
                <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                  ...
                  <div style={{ maxWidth: "72%", background: msg.role === "user" ? "#00AEEF" : "white", color: msg.role === "user" ? "white" : "#111827", borderRadius: 12, padding: "12px 16px", fontSize: 14, lineHeight: 1.6, border: msg.role === "assistant" ? "1px solid #e5e7eb" : "none", whiteSpace: "pre-wrap" }}>
                    {msg.content}
                  </div>
                </div>
              ))}
        ...
        <div style={{ background: "white", borderTop: "1px solid #e5e7eb", padding: "16px 20px" }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
            ...
            <button onClick={() => sendMessage()} disabled={!input.trim() || loading} style={{ background: input.trim() && !loading ? "#00AEEF" : "#d1d5db", border: "none", borderRadius: 10, width: 44, height: 44, cursor: input.trim() && !loading ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.15s", flexShrink: 0 }}>
